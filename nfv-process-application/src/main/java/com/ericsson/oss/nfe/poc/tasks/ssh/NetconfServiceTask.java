...
public class NetconfServiceTask implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		if (licenseCheck()) {
			if (expressionCheck(execution)) {
				...
				log.info("Netconf Started expected hello-->\n" + outputCmd);
				...
				if (!"".equalsIgnoreCase(filterStr)  )
				{
					log.info("Netconf command 2 to be executed -->\n"
							+ filterStr);
					String result2 = executeCommand(filterStr);
			...
			//	log.info("Netconf executed and output -->\n" + result2);
				...
				execution.setVariable("netconfOutput", result);
		...
		case SshAuthentication.COMPLETE: {
			this.sshSession = this.sshClient.openSessionChannel();
			 if(sshSession instanceof Ssh2Session)
				    ((Ssh2Session)sshSession).startSubsystem("netconf");
				 else
				    throw new Exception("Wrong SSH version");
	...
	private boolean expressionCheck(DelegateExecution execution) {
		if (isValidExpression(netconfHost, execution)
			...
			port = Utils.toInt(
					netconfPortNumber.getValue(execution).toString(), 830);
